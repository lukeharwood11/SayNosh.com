...
show_help() {
  printf '%s\n' \
    "Usage: push-linked-migrations.sh [--linked|--local] [additional supabase db push flags]" \
    "" \
    "Defaults to --linked so it uses the project configured by 'supabase link'." \
    "Pass --local to push to the local Supabase database instead." \
    "All remaining arguments are forwarded to 'supabase db push'."
}
...
printf '%s\n' "Running: supabase db push $target_flag $*"

cd "$REPO_ROOT"
supabase db push "$target_flag" "$@"
